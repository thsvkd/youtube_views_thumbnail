...
def make_thumbnail_image(text, font_size, font_color, font_style, img_name, img_size):
    ...
    BG_img = cv2.imread(
        "C:/Users/%s/Desktop/%s" % (user_name, img_name))

    BG_img = image_resize(BG_img, img_size)

    font = get_font_truetype(
...
if __name__ == "__main__":
    ...
    if DEBUG:
        final_thumbnail = make_thumbnail_image(text="이 영상의 조회수는 123입니다", font_size=52, font_color="ffd36b",
                                               font_style="NotoSansCJKjp-Bold", img_name="IMG_2330.JPG", img_size=0.1)
